...
open class BatchingBase {
    ...
    fun dataStream(): Flux<Byte> = Flux.range(0, 99).map { i: Int -> i.toString().toByte() }
    ...
    fun inputCommandStream(): Flux<Command> = Flux.range(0, 100)
    ...
    fun metrics(): Flux<Long> = Flux.interval(Duration.ofMillis(95))
}
