...
class c9_ExecutionControl : ExecutionControlBase() {
    ...
    fun slow_down_there_buckaroo() {
        ...
        val notifications = readNotifications()
            .doOnNext { x: String? -> println(x) } //todo: change this line only
        StepVerifier.create(notifications
            .doOnNext { s: String? -> assertThread(threadId) })
            .expectNextCount(5)
            .verifyComplete()
    }
    ...
    fun blocking() {
        ...
        val task = Mono.fromRunnable<Any> { blockingRunnable() } //todo: change this line only
            .then()
        StepVerifier.create(task)
            .verifyComplete()
    }
    ...
    fun free_runners() {
        //todo: feel free to change code as you need
        val task = Mono.fromRunnable<Void>(blockingRunnable())
        val taskQueue = Flux.just(task, task, task)
            .concatMap(Function.identity())

        //don't change code below
        val duration = StepVerifier.create(taskQueue)
            .expectComplete()
            .verify()
    ...
    fun sequential_free_runners() {
        ...
        //don't change code below
        val duration = StepVerifier.create(tasks)
            .expectNext("1")
            .expectNext("2")
            .expectNext("3")
            .verifyComplete()
    ...
    fun event_processor() {
        //todo: feel free to change code as you need
        val eventStream = eventProcessor()
            .filter { event: Event -> event.metaData.length > 0 }
            .doOnNext { event: Event -> println("Mapping event: " + event.metaData) }
        ...
        StepVerifier.create(eventStream)
            .expectNextCount(250)
            .verifyComplete()
        val steps = Scannable.from(eventStream)
            .parents()
            .map { obj: Any -> obj.toString() }
            .collect(Collectors.toList())
        val last = Scannable.from(eventStream)
            .steps()
