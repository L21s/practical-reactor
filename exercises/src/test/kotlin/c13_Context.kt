...
class c13_Context : ContextBase() {
    ...
    fun message_tracker() {
        ...
        val mono = messageHandler("Hello World!")
            .contextWrite(Context.of(HTTP_CORRELATION_ID, "2-j3r9afaf92j-afkaf"))
        StepVerifier.create(mono)
            .expectNextMatches { m: Message -> m.correlationId == "2-j3r9afaf92j-afkaf" && m.payload == "Hello World!" }
            .verifyComplete()
    }
    ...
    fun execution_counter() {
        val repeat = Mono.deferContextual { ctx: ContextView ->
    ...
    fun pagination() {
        ...
        val results = getPage(1)
            .flatMapMany(Page::result)
            .repeat(10)
            .doOnNext { i: Int -> println("Received: $i") }
