...
class c4_LifecycleHooks : LifecycleHooksBase() {
    ...
    fun be_there_early() {
        val hooksTriggered = CopyOnWriteArrayList<String>()
        val temperatureFlux = room_temperature_service() //todo: change this line only
        StepVerifier.create(temperatureFlux.take(5).doOnSubscribe { s: Subscription? -> hooksTriggered.add("subscribe") })
            .expectNextCount(5)
            .verifyComplete()
    ...
    fun ordering_is_important() {
        ...
        val orderOfExecution = Arrays.asList("todo", "todo", "todo") //todo: change this line only
